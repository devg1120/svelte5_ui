<script lang="ts">
  ...
  let { highlightedDates, theme, validRange, _interaction, _today } =
    getContext("state");

  let el = $state();
  let dayChunks = $derived(
      chunks.filter((chunk) => datesEqual(chunk.date, date)),
    ),
    dayBgChunks = $derived(
      bgChunks.filter((bgChunk) => datesEqual(bgChunk.date, date)),
    );
