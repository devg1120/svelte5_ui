<script lang="ts">
    ...
    let isToday = $derived(datesEqual(date, $_today)), otherMonth = $derived(date.getUTCMonth() !== $currentDate.getUTCMonth()), highlight = $derived($highlightedDates.some(d => datesEqual(d, date))), disabled = $state();
