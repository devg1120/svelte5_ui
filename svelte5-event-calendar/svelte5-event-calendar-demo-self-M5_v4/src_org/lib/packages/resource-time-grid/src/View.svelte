      ...
      <div class={$theme.resource}>
        ...
        {:else}
          <div class={$theme.day}>
            <Label
              resource={item0}
              on:text={(e) => (resourceLabels[i] = e.detail + ", ")}
            />
          </div>
        {/if}
        {#if loops[1].length > 1}
          <div class={$theme.days}>
            {#each loops[1] as item1}
              ...
              {:else}
                <div
                  class="{$theme.day} {$theme.weekdays?.[item1.getUTCDay()]}"
                  role="columnheader"
                >
                  <time
                    datetime={toISOString(item1, 10)}
                    aria-label="{resourceLabels[i]}{$_intlDayHeaderAL.format(
                      item1,
                    )}"
